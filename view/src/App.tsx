...
import { PlanSuggestion, suggestNext, WorkoutState, logSet, updateLoggedSet, deleteLoggedSet } from "./workoutFlows"
import { init, fetchEvents, insertEvent, updateEvent as persistUpdatedEvent, removeEvent } from "./storage"
...
const App = () => {
  ...
  useEffect(() => {
    ...
    suggestNext({ events: state.events } as WorkoutState, state.suggestions.planner)
  ...
  const actions = useMemo(
    () => ({
      navigate: (screen: typeof state.nav.screen) => dispatch({ type: "nav/set", screen }),
      setSelectedDate: (date: Date) => dispatch({ type: "date/set", date }),
      shiftDate: (deltaDays: number) => dispatch({ type: "date/shift", deltaDays }),
      refreshAll: async () => {
        await refreshFromStorage()
        await refreshCatalog()
      },
      ...
      openLogForExercise: (exerciseName: string | undefined, date: Date, tab: typeof state.logging.tab) => {
        dispatch({ type: "log/date", date })
        ...
        const matching = exerciseName
          ? state.events
              .filter((event) => String(event.payload?.exercise ?? "") === exerciseName)
        ...
        if (matching) {
          dispatch({
            type: "log/fields",
            fields: {
              reps: typeof matching.payload?.reps === "number" ? matching.payload.reps.toString() : "",
              weight: typeof matching.payload?.weight === "number" ? matching.payload.weight.toString() : "",
              duration: typeof matching.payload?.duration === "number" ? matching.payload.duration.toString() : "",
              distance: typeof matching.payload?.distance === "number" ? matching.payload.distance.toString() : "",
            },
          })
        } else {
          dispatch({ type: "log/fields", fields: { ...initialFields } })
      ...
      logSet: async (payload: { exercise: string; reps?: number; weight?: number; duration?: number; distance?: number }) => {
      ...
      updateSet: async (
        eventId: string,
        payload: { exercise: string; reps?: number; weight?: number; duration?: number; distance?: number },
      ) => {
        const nextState = await updateLoggedSet({ events: state.events } as WorkoutState, eventId, payload)
        dispatch({ type: "events/set", events: nextState.events })
        const updated = nextState.events.find((event) => event.event_id === eventId)
        if (updated) {
          await persistUpdatedEvent(updated)
        }
      },
      deleteSet: async (eventId: string) => {
        const nextState = deleteLoggedSet({ events: state.events } as WorkoutState, eventId)
        dispatch({ type: "events/set", events: nextState.events })
        await removeEvent(eventId)
      },
      saveCustomExercise: async (values: ExerciseCatalogEntry, originalSlug?: string) => {
    ...
    [refreshCatalog, refreshFromStorage, state.events, state.logging.logDate, state.nav.screen, state.suggestions.planner],
  )
