...
- ### FitNotes UI Blitz (current focus)
  ...
  - [ ] **Logging polish + defaults**
    - Align steppers, segmented tabs, and set table spacing with the provided screenshots.
    - Prefill weight/reps from the last logged set for that exercise and show inline toast/status banners (Training saved, etc.).
  - [ ] **Graph / analytics parity**
...
- [ ] **Guided Logging Wizard**
  ...
  - [ ] Step 4: Exercise logging screen enhancements
    - [x] Adapt inputs to `logging_mode` with weight/reps/time/distance steppers.
    - [x] Show contextual date header + Track/History/Trends tabs (Today list + grouped history + inline chart).
